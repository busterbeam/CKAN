...
from click import command, option, argument, INT
...
class CkanPullRequest:
	...
	def changelog_entry(self) -> str:
		return (
			f'- [{self.badge()}] {self.pull_request.title}",
			f" (#{self.pull_request.number} by: ",
			f"{self.pull_request.user.login}; reviewed: ",
			f"{', '.join(self.approvers())})\n')
	
	def latest_commit(self, repo: CkanRepo) -> Commit:
		print((
			f"Fetching {self.pull_request.head.repo.clone_url}",
			f"{self.pull_request.head.ref} . . ."))
		repo.git.fetch(
			self.pull_request.head.repo.clone_url, self.pull_request.head.ref)
	...
	def merge_into(self, repo: CkanRepo) -> bool:
		...
		# repo.index.commit doesn't properly resolve a merge
		# started via repo.git.merge
		repo.git.commit(m=self.merge_commit_message())
		return True

@command()
@option('--repo-path', type=cPath(exists=True, file_okay=False),
		default='.', help='Path to CKAN working copy')
@option('--token', required=False, envvar='GITHUB_TOKEN')
@argument('pr_num', type=INT)
def merge_pr(repo_path: str, token: str, pr_num: int) -> None:
	...
	exit(ExitStatus.success if ckpr.merge_into(ckr) else ExitStatus.failure)
